...
int* findSubstring(char* s, char** words, int wordsSize, int* returnSize) {
	...
	data** map = (data**)malloc(sizeof(data*) * 1000);
	data** tmp = (data**)malloc(sizeof(data*) * 1000);
	int i, j;
	for (i = 0; i < 1000; i++)
	{
		map[i] = NULL;
	...
	*returnSize = 0;
	...
	for (i = 0; i < 1000; i++) if (map[i] != NULL) free(map[i]);
	free(map);
	return result;
}
...
{
	int i;
	char s[] = "wordgoodstudentgoodword";
	char *words[] = {"word","student"};
